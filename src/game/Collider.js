...
(function(Collider, Vector){
    Collider.create = function(options){
        ...
        var collides = function(collider){
            ...
            var responseX = ((that.w/2 + collider.w/2) - Math.abs(otherCenterX - centerX));
            var responseY = ((that.h/2 + collider.h/2) - Math.abs(otherCenterY - centerY));
            
            if (centerX < otherCenterX) {responseX *= -1};
            if (centerY < otherCenterY) {responseY *= -1};
